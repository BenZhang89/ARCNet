...
import sys
# sys.path.insert(0, '/gpfs/scratch/bz957/brats_skynet/lib/python3.6/site-packages/')
import numpy as np
...
class SegPostprocessor:
    def __init__(self, input_dir, output_dir, config, num_workers):
        self.input_dir = input_dir
        self.config = config
        self.output_dir = output_dir
        self.num_workers = num_workers
        self.acc = os.path.basename(input_dir)
        self.bk_modelities=['t1ce','flair']
        self.pacserver="RESEARCH@10.147.124.19:104"
    def nifti2dicom_pool(self):
        """
        convert nifti to dicom for sending back to PACS
        """
        #nifti2dicom(seg_nifti, background_nifti, ref_dicom_dir, save_dir, description, mode_RGB=True, zoom_num=2, watermarks=True)

        seg_nifti = os.path.join(self.output_dir, 'nifti/nifti_seg_whole.nii.gz')
        background_nifti = [os.path.join(self.input_dir,'nifti/' + i + '_coreg.nii.gz') for i in self.bk_modelities] 
        ref_dicom_dir = os.path.join(self.input_dir, 't1ce')
        save_dir = [os.path.join(self.output_dir, i) for i in self.bk_modelities]
        description = [i+'_tumor_segmentations' for i in self.bk_modelities]

        #multiprocess for each Nifty files
        pool = multiprocessing.Pool(self.num_workers)
        args_list = []
        for i, bk in enumerate(background_nifti):
            args_list.append([seg_nifti, background_nifti[i], ref_dicom_dir, save_dir[i],description[i]])
        print(args_list)
        pool.starmap(nifti2dicom, args_list)

    def push2docker(self):
        """
        copy dicoms to docker server
        """
        dicom2push=[os.path.join(self.output_dir, i) for i in self.bk_modelities]
        for dicom_src in dicom2push:
            dicom_dst = dicom_src.replace("t1ce","t1cetest")
            #subprocess.call(["cp","-r", dicom_src, dicom_dst])
            shutil.copytree(dicom_src, dicom_dst)
    def push2pacs(self):
        """
        push dicoms to PACS research server
        """
        dicom2push=[os.path.join(self.output_dir, i) for i in self.bk_modelities]
        for dicom in dicom2push:
            print(dicom)
            subprocess.call(["dcm4che-5.22.6/bin/storescu", "-c", self.pacserver, dicom])
    ...
    def test(self):
        ...
        for i in range(image_num):
            ...
            if(config_net1):
                data_shapes  = [ data_shape1[:-1],  data_shape1[:-1],  data_shape1[:-1]] #why not use [19, 180, 160] instead of [19, 180, 160, 4] in input
            ...
            if(config_test.get('whole_tumor_only', False) is True): #改动3!
                ...
                if('brain_cavity' in config_data['modality_postfix'] and 'mha' in config_data['file_postfix']):
                    out_label[label2>0] = 3
                    out_label[label3==1] = 1
                    out_label[label3==2] = 4
                #incorporate cavity into the tumor core
                elif('brain_flair' in config_data['modality_postfix'] and 'nii' in config_data['file_postfix']):
            ...
            save_array_as_nifty_volume(final_label, subfolder+"/{}_seg_whole.nii.gz".format(temp_name.split('/')[-1]), img_names[0])
            tumor_volume = calculate_tumor(subfolder+"/{}_seg_whole.nii.gz".format(temp_name.split('/')[-1]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input', type=str, default='/gpfs/data/luilab/BRATS/data/incoming/acc#',
    ...
    parser.add_argument('-o', '--output', type=str, default='/gpfs/data/luilab/BRATS/data/processesed',
        help='location of the saving nifti and dicom files')
    parser.add_argument('-c', '--config', type=str, default='/gpfs/data/luilab/BRATS/brats/config19/test_all_class.txt',
        help='location of the processesed dicom&nifti files')
    parser.add_argument('-n', '--num_workers', type=int, default=2, help='Number of workers')

    args = parser.parse_args()
    #check existence of input files
    skullstripped_brains = args.input + '/nifti'
    if not os.path.exists(skullstripped_brains):
        print('Input folder not exists')
        exit()
    #else:
    #    if len(os.listdir(skullstripped_brains))==4:
    #        print(f'Input series are {os.listdir(skullstripped_brains)}')
    #    else:
    #        print('Input series should be t1ce.nii.gz, t1.nii.gz, t2.nii.gz, flair.nii.gz, but got:{os.listdir(args.input)}')
    #        exit()


    #brain tumor segmentation, nifti to dicom
    one_acc = SegPostprocessor(args.input,args.output,args.config, args.num_workers)
    one_acc.test()
    one_acc.nifti2dicom_pool()
    #one_acc.push2pacs()
