...
class DataLoader(Dataset):
    def __init__(self, phase="train", config=None):
        """
        ...
        if phase == "train":
            self.data_names = config.get('train_names', None)
        elif phase == "validate":
            self.data_names = config.get('val_names', None)
        else:
            self.data_names = config.get('test_names', None)
    ...
    def __getitem__(self, index):
        ...
        # return batch sized volume: [batch_size=1, moda_chnl, slice_d, slice_h, slice_w]
        ...
        data_volumes = [x for x in self.data[index]]
        boundingbox = None
        if(self.with_ground_truth):
            label_volumes = [self.label[index]]
            if(train_with_roi_patch):
        ...
        sub_data = np.asarray(sub_data,np.float32)
        ...
        data_batch = torch.from_numpy(sub_data)
        #print(f'data batch is {data_batch.shape}')
        label_batch = torch.from_numpy(sub_label)
        #convert label from (D,H,W) to (C,D,H,W)
        #label_batch = torch.unsqueeze(label_batch, 0)
        #print(f'label batch is {label_batch.shape}')
    ...
    def __get_patient_names(self):
        ...
        if(self.data_names is not None):
            ...
            if os.path.isfile(self.data_names):
    ...
    def load_data(self):
        ...
        """
        if self.data_names:
            self.patient_names = self.data_names
        else:
        ...
        for i in range(data_num):
            ...
            for mod_idx in range(len(self.modality_postfix)):
                print('Loading patient {} and modality {}'.format(self.patient_names[i],self.modality_postfix[mod_idx]))
