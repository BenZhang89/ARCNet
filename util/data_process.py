...
def set_ND_volume_roi_with_bounding_box_range(volume, bb_min, bb_max, sub_volume):
    ...
    if(dim == 2):
        ...
        out[np.ix_(range(bb_min[0], bb_max[0]  + 1),
                   ...
                   range(bb_min[2], bb_max[2]  + 1))] = sub_volume #改动2   #+ 1
    elif(dim == 4):
        out[np.ix_(range(bb_min[0], bb_max[0]  + 1),
                   range(bb_min[1], bb_max[1]  + 1),
...
def add_tumor_volume_to_nifty(filename, volume):
    """
...
def nifti2dicom(seg_nifti, bk_nifti, ref_dicom_dir, save_dir, description, mode_RGB=False, zoom_num=4, watermarks=True):
    """
    add covert nifti to dicoms using referencing
    """    
...
def get_contours(img):
    canvas = np.zeros(list(img.shape), dtype=np.uint8)
    if np.all(img==0):
        return canvas
    else:
        #whole tumor
        img_edema = img.copy()     
        img_edema[img_edema == 1] = 2
        img_edema[img_edema == 4] = 2
        
        #tumor core
        img_core = img.copy()     
        img_core[img_core == 2] = 0
        img_core[img_core == 4] = 1
        
        #enhancing tumor
        img_enhancing = img.copy()     
        img_enhancing[img_enhancing == 2] = 0
        img_enhancing[img_enhancing == 1] = 0
        canvas = np.zeros(list(img.shape), dtype=np.uint8)
        for img_i in [img_edema, img_core, img_enhancing]:
            if not np.all(img_i==0):
                #set different values for different contours
                if np.unique(img_i)[-1] == 1:
                    color = 1
                elif np.unique(img_i)[-1] == 2:
                    color = 2
                else:
                    color = 4
                contours, h = cv2.findContours(img_i,cv2.RETR_TREE,cv2.CHAIN_APPROX_NONE)
                #draw the RGB boundary
                cv2.drawContours(canvas, contours, -1, color, 1)
        
        return canvas

def toRGB(img):
    #convert image_slice to fit RGB dicom 
    img = img/img.max() * 255
